...
ofxAutoReloadedShader::ofxAutoReloadedShader() : type(TYPE::other), bWatchingFiles(false), _need_geom(false) {}
...
bool ofxAutoReloadedShader::load(const std::filesystem::path& shaderName, const bool need_geom)
{
	_need_geom = need_geom;
	return load(shaderName.string() + ".vert", shaderName.string() + ".frag", shaderName.string() + ".geom", need_geom);
}

bool ofxAutoReloadedShader::load(
	const std::filesystem::path& vertName, const std::filesystem::path& fragName, const std::filesystem::path& geomName
	, bool need_geom)
{
	unload();
	...
	if (need_geom && !geometryShaderFilename.empty())
...
bool ofxAutoReloadedShader::reloadShaders()
{
	if (type == TYPE::compute)
		...
		return load(vertexShaderFilename, fragmentShaderFilename, geometryShaderFilename, _need_geom);
}
...
std::time_t ofxAutoReloadedShader::getLastModified(ofFile& _file)
{
	if (_file.exists())
	{
		auto ftime = std::filesystem::last_write_time(_file.path());
